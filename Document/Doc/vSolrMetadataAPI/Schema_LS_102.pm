package Document::Doc::vSolrMetadataAPI::Schema_LS_102;
...
sub post_process_metadata {
    ...
    # ht_id_display is an increasingly long, pipe-delimited field with item-level information derived from
    # a combination of the record-level metadata and whatever metadata is passed along in the 974 for
    # each item.
    #
    # NOTE: These fields are produced _exclusively_ for use by LS/CB. They should *not* be massaged in this
    # code, but rather changed in the catalog indexing code for easy comsumption here.
    #
    # Source of truth is at (or near) https://github.com/hathitrust/hathitrust_catalog_indexer/blob/641238a8f3cdd38f43483345aad89d7224252600/indexers/common_ht.rb#L73
    #
    # Configuration is:
    #  0. htid
    #  1. last_update_date
    #  2. enum_chron
    #  3. enum pubdate (as determined by the post-zephir processing code, so might be too conservative)
    #  4. enum_pudate_range (e.g, "1910-1919")
    #  5. title_sortkey
    #  6. author_sortkey
    #  7. title_display
    ...
    my ($_htid, $last_update_date, 
        $volume_enumcron, $enum_date, $enum_range, 
        $title_sortkey, $author_sortkey,
	$title_display) = split(/\|/, $hathiTrust_str[0]);
    ...
    # Dump the sortkeys and title display into the metadata
    $metadata_hashref->{title_sortkey} = [$title_sortkey];
    $metadata_hashref->{author_sortkey} = [$author_sortkey];   
    $metadata_hashref->{title_display} = [$title_display];
